...
public class ShootMethod extends BoundaryTask {
    ...
    public double[] resolve(int scale, RoundingMode roundingMode) {
        ...
        VariableValuePair[] pair = {new VariableValuePair("yx", initialValue), new VariableValuePair("sx", shootingAngles[0])};
        RungeKutta rungeKutta = new RungeKutta(fstr, pair, a, b, h);
        double[][] res1 = rungeKutta.resolve(scale, roundingMode);
        rungeKutta.setInitialValue(2, shootingAngles[1]);
        double[][] res2 = rungeKutta.resolve(scale, roundingMode);
        double y0 = alpha2 * res1[0][n - 1] + beta2 * res1[1][n - 1] - gamma2, y1 = alpha2 * res2[0][n - 1] + beta2 * res2[1][n - 1] - gamma2, y2;
        ...
        if (y0 * y1 < 0) {
            ...
            while (iter < 1000) {
                ...
                res1 = rungeKutta.resolve(scale, roundingMode);
