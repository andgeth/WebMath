...
import main.java.by.vsu.diffeq.BetterEuler;
...
@Controller
public class BetterEulerController {

    @RequestMapping("/betterEuler")
    public String euler(
        ...
        if (function != null) {
            ...
            BetterEuler betterEuler;
            VariableValuePair vvPair = new VariableValuePair(y0);
            try {
                betterEuler = new BetterEuler(function, vvPair, a, b, h);
            } catch (Exception ex) {
                model.addAttribute("function", function);
                model.addAttribute("interval", interval);
                model.addAttribute("y0", y0);
            ...
            double[] answer = betterEuler.resolve(5, RoundingMode.UP);
