...
public class Matrix {
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.n-1; i++) {
            sb.append(String.format(locale, "%."+this.precision+"f" , this.M[i][0]));
            for (int j = 1; j < this.m; j++) {
                sb.append(String.format(locale, " %."+this.precision+"f" , this.M[i][j]));
            }
            sb.append("\n");
        }
        sb.append(String.format(locale, "%."+this.precision+"f" , M[this.n - 1][0]));
        for (int j = 1; j < this.m; j++) {
            sb.append(String.format(locale, " %."+this.precision+"f" , M[this.n - 1][j]));
        }
        return sb.toString();
    }
